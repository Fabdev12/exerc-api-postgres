...
class Order {
  ...
  static async create(customerId, orderProducts) {
    ...
    const storedOrderProducts = await query(
      `SELECT * FROM products WHERE id = ANY($1::int[]);`, //onde $1 possui um array de inteiros
    ...
    let response
    try {
      await dbClient.query("BEGIN") //Inicia uma transaction
      ...
      //insere um registro para cada produto
      ...
      await dbClient.query("ROLLBACK")//desfaz tudo(em caso de erro)que vem depois do BEGIN, seguindo o padrão de uma trasaction
